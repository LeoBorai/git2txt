...
- [ ] 1.1 Change `Cli.url: String` to `Cli.source: String` with `default_value = "."` in
      `src/bin/main.rs`, and verify `cargo run --` (no args) no longer errors with "required
      argument missing"
- [ ] 1.2 Implement a `source` classifier (remote URL vs. local path): remote if it starts with
...
- [ ] 1.3 Branch `main` on the classifier: remote → existing `Git::download` into a `TempDir`
...
- [ ] 1.4 Validate the local scan target exists and is inside a Git repository (e.g. via
      ...
      written
- [ ] 1.5 Handle the unresolvable-source case: `source` is not an existing local path and not a
...
- [ ] 2.1 Replace `Scanner`'s recursive `read_dir` walk in `src/scanner.rs` with a function that
      ...
      warnings
- [ ] 2.2 Filter out a root-level file literally named `output.txt` from the enumerated list
...
- [ ] 2.3 For each enumerated path, read its bytes and append them into the aggregate output file
...
- [ ] 2.4 Propagate `git ls-files` failure as an `anyhow::Error` that aborts before any output
...
- [ ] 3.1 Wire sections 1 and 2 together in `main.rs`: resolve `source` → scan directory →
...
- [ ] 3.2 Run `cargo clippy` and `cargo fmt --all -- --check` and fix any new warnings introduced
      by the refactor, matching CI's `clippy.yml`/`fmt.yml` checks
- [ ] 3.3 Update `README.md`'s "Running" section to document the new `source` argument
